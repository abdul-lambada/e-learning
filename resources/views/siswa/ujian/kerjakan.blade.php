...
@section('content')
    ...
    <div class="exam-header d-flex justify-content-between align-items-center">
        <div>
            <h5 class="m-0 fw-bold">{{ $jawabanUjian->ujian->nama_ujian }}</h5>
            <small class="text-muted">Percobaan ke-{{ $jawabanUjian->percobaan_ke }}</small>
        </div>
                                        ...
                                        @foreach (['A', 'B', 'C', 'D', 'E'] as $opt)
                                            @if ($detail->soalUjian->{'pilihan_' . strtolower($opt)})
                                                ...
                                                <label for="opt_{{ $detail->id }}_{{ $opt }}"
                                                    ...
                                                    {{ $detail->soalUjian->{'pilihan_' . strtolower($opt)} }}
                                                </label>
                                            @endif
                                        @endforeach
                                    @else
    ...
    <script>
        // Config
        const totalQuestions = {{ $jawabanUjian->detailJawaban->count() }};
